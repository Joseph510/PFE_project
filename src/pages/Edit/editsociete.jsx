...
const Edit3 = ({ inputs, title }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const [values, setValues] = useState(
    location.state.societe
      ? {
          ...location.state.societe,
        }
      : {}
  ...
  const handle = (e) => {
    ...
    axios.put(`http://localhost:4000/societe/update/${values.id}`, values,{   
         headers: {
        'Content-Type': 'application/json'
      }}).then((res) => {
        console.log(res.data);
        swal("Success", "La Societe a été modifié avec succès", "success");
        navigate('/Societe');
      })
      .catch(() => {
        console.log("failed");
